...
ecospat.niche.equivalency.test2 <- function (z1, z2, rep, alternative = "greater", ncores = 1) 
{
  ...
  if (ncores == 1) {
    ...
    clusterEvalQ(cl = cl, 
                 expr = library('ecospat'))
    sim.o <- as.data.frame(matrix(unlist(parLapply(cl, 1:rep, 
                                                   overlap.eq.gen, z1, z2)), byrow = TRUE, ncol = 2))
    stopCluster(cl)
  }
...
ecospat.niche.similarity.test2 <- function (z1, z2, rep, 
                                            alternative = "greater", 
                                            rand.type = 1,
                                            ncores = 1) 
{
  ...
  if (ncores == 1) {
    ...
    cl <- makeCluster(ncores)
    clusterEvalQ(cl = cl, 
                 expr = c(library('ecospat'),
                          library('progress')))
    pb <- utils::txtProgressBar(min = 0, max = rep, 
                                style = 3)
